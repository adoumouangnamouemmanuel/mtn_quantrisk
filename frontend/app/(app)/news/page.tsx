...
import {
  ...
  ChevronRight, X, ExternalLink, Brain, Shield, Wifi,
  Globe, Activity, Eye
} from 'lucide-react';
...
function ArticleCard({ article, onClick }: { article: NewsArticle; onClick: () => void }) {
  ...
  return (
    <button
      onClick={onClick}
      className="w-full text-left rounded-xl border p-4 transition-all duration-150 hover:border-mtn-yellow/30 group relative overflow-hidden"
      style={{ background: 'rgba(255,255,255,0.02)', borderColor: 'rgba(255,255,255,0.07)' }}
    >
      {/* tier stripe */}
      {tier && <div className={`absolute left-0 top-0 bottom-0 w-1 rounded-l-xl ${tier.bar}`} />}

      <div className="pl-2">
        {/* header row */}
        <div className="flex items-start gap-2 mb-2">
          <div className="flex-1 min-w-0">
            <p className="text-sm font-semibold text-on-surface leading-snug line-clamp-2 group-hover:text-mtn-yellow transition-colors">
              {article.title}
            </p>
          </div>
          <ChevronRight className="w-4 h-4 text-on-surface-variant shrink-0 mt-0.5 group-hover:text-mtn-yellow transition-colors" />
        </div>
        ...
        <div className="flex flex-wrap items-center gap-2 text-xs text-on-surface-variant">
          ...
          {article.alertTier && (
            <span className={`px-2 py-0.5 rounded-full font-mono font-bold text-xs border border-current ${tier?.label ?? ''}`}>
              {article.alertTier}
            </span>
          )}

          {article.sentiment && (
            <span className={`px-2 py-0.5 rounded-full font-mono text-xs ${SENTIMENT_STYLE[article.sentiment] ?? ''}`}>
              {article.sentiment}
            </span>
          )}

          {article.impactGhsMid != null && (
            <span className="font-mono text-mtn-yellow">
              ~{fmtGhs(article.impactGhsMid)} impact
            </span>
          )}
        </div>
      </div>
...
function ArticleDrawer({ article, onClose }: { article: NewsArticle; onClose: () => void }) {
  const cat = article.category ? CATEGORY_META[article.category] : null;
  const tier = article.alertTier ? TIER_STYLE[article.alertTier] : null;
  ...
  return (
    <div className="fixed inset-0 z-50 flex justify-end">
      ...
      {/* panel */}
      <div className="relative w-full max-w-xl h-full overflow-y-auto shadow-2xl flex flex-col"
        ...
        <div className="sticky top-0 z-10 flex items-center justify-between px-5 py-4 border-b"
          ...
          <div className="flex items-center gap-2">
            {cat && (
              <span className={`flex items-center gap-1 px-2 py-0.5 rounded-full border font-mono text-xs font-medium ${cat.color} ${cat.bg}`}>
                {cat.icon} {cat.label}
              </span>
            )}
            {article.alertTier && (
              <span className={`px-2 py-0.5 rounded-full font-mono text-xs font-bold border border-current ${tier?.label ?? ''}`}>
                {article.alertTier}
              </span>
            )}
          </div>
          <button onClick={onClose} className="p-1.5 rounded-lg hover:bg-white/05 text-on-surface-variant hover:text-on-surface transition-colors">
            <X className="w-4 h-4" />
          </button>
        </div>
        ...
        <div className="flex-1 px-5 py-5 space-y-5">
          <h2 className="text-lg font-hero font-bold text-on-surface leading-snug">{article.title}</h2>

          {/* meta */}
          <div className="flex flex-wrap gap-3 text-xs text-on-surface-variant font-mono">
            <span>{article.sourceName ?? 'Unknown'}</span>
            <span>·</span>
            <span>{fmtDate(article.publishedAt)}</span>
            {article.sentiment && (
              <span className={`px-2 py-0.5 rounded-full ${SENTIMENT_STYLE[article.sentiment] ?? ''}`}>
                {article.sentiment}
              </span>
            )}
          </div>
          ...
          {(article.severity != null || article.mtnRelevance != null || article.impactGhsMid != null) && (
            <div className="rounded-xl border p-4 space-y-3" style={{ borderColor: 'rgba(255,208,0,0.15)', background: 'rgba(255,208,0,0.03)' }}>
          ...
          {article.body && (
            <div className="space-y-2">
              <p className="text-xs font-mono uppercase tracking-widest text-on-surface-variant">Article Body</p>
          ...
          <a href={article.url} target="_blank" rel="noopener noreferrer"
            className="flex items-center gap-1.5 text-xs font-mono text-mtn-yellow hover:underline">
      ...
      <div className="flex justify-between items-end">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-on-surface-variant mt-0.5">Live-scraped Ghanaian news classified for MTN risk exposure</p>
          </div>
        </div>
        <button
