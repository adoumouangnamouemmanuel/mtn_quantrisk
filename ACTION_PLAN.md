...
|---|---|---|---|---|---|
| C1 | **Revoke the exposed GitHub Personal Access Token** | `.git/config` | 30 min | Anyone with repo access can use the exposed PAT to push to the account. Revoke at github.com/settings/tokens, then remove it from the remote URL. **⚠️ CREDENTIAL REDACTED — revoke immediately.** | 🔲 (user action — revoke at GitHub) |
...
| C3 | **Add authentication to the backend API** | `backend/app/core/security.py`, `backend/app/api/auth.py`, `backend/app/main.py` | 1 day | **Completed.** Implemented local JWT auth (HS256, stdlib-only) with `POST /api/auth/login`, `GET /api/auth/me`, `POST /api/auth/logout`. All `/api/*` routes now require `Authorization: Bearer <token>`. **⚠️ Default credentials removed — set AUTH_EMAIL and AUTH_PASSWORD in environment.** | ✅ Done |
